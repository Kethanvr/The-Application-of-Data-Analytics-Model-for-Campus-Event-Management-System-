...
def create_event(request, event_id=None):
    ...
    if request.method == 'POST':
        try:
            if event:  # Updating existing event
                ...
                event.expected_attendees = request.POST['expected_attendees']
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'success': True, 
                    'message': 'Event updated successfully!' if event_id else 'Event created successfully! Waiting for approval.',
                    'event_id': event_id
                })
            ...
            context = {'event': event} if event_id else {}
            return render(request, 'events/create_event.html', context)
    
    # For GET requests, pass the event to the template if editing
    context = {'event': event} if event_id else {}
    return render(request, 'events/create_event.html', context)
...
def online_events(request):
    """View for students to see their registered online events and access meeting links"""
    if request.user.role != 'STUDENT':
        messages.error(request, 'Access denied.')
        return redirect('dashboard')
    
    # Get online events the student is registered for
    registered_online_events = EventRegistration.objects.filter(
        student=request.user,
        event__mode='ONLINE'
    ).select_related('event')
    
    context = {
        'registered_online_events': registered_online_events
    }
    
    return render(request, 'events/online_events.html', context)
...
def event_scheduler(request):
    ...
    # Get all approved ONLINE events for this coordinator
    events = Event.objects.filter(
        coordinator=request.user,
        status='PRINCIPAL_APPROVED',
        mode='ONLINE'  # Filter for online events only
    ).order_by('-date')
    
    # Get existing schedules for online events only
    event_schedules = EventSchedule.objects.filter(
        event__coordinator=request.user,
        event__mode='ONLINE'  # Filter for online events only
    ).order_by('event__event_name', 'start_time')
    
    if request.method == 'POST':
        try:
            event_id = request.POST.get('event')
            event = get_object_or_404(Event, id=event_id, coordinator=request.user, mode='ONLINE')
            ...
            # Update the event's meeting link
            if meeting_link:
